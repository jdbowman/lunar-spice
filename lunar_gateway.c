...
int main (int argc, char** argv) {
  ...
  SpiceDouble vecRefSiteNorth_MoonME[3];
  SpiceDouble vecRefSiteEast_MoonME[3];
  ...
  SpiceDouble latlon[2] = {0.0, 180.0};
  ...
  while ((opt = getopt(argc, argv, "d:f:g:hs:l:n:o:")) != -1) {
    ...
    switch (opt) {
      ...
      case 'l':
        latlon[0] = atof(optarg);
        break;

      case 'g':
        latlon[1] = atof(optarg);
        break;
  ...
  for (int n=0; n<numSteps; n++) {
    ...
    latsrf_c("Ellipsoid", "MOON", et, "MOON_ME", 1, &latlon, &posRefSite_MoonME);
    ...
    // Get local north and east vectors for the reference site
    ...
    // Calculate zenith angle of Gateway viewed from reference site
		// zenithAngle = dpr_c()*vsep_c(vecRefSiteNormal_MoonME, vecRefSiteToGateway_MoonME);
